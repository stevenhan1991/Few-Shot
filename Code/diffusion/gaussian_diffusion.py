...
class GaussianDiffusion:
    ...
    def ddpm_sample(self,model,shape,cond,clip_denoised=True,denoised_fn=None,model_kwargs=None):

        x_t = th.randn(*shape, device=cond.device)

        for time_step in reversed(range(self.num_timesteps)):
            t = x_t.new_ones([shape[0], ], dtype=torch.long) * time_step
            out = self.p_mean_variance(model,x_t, t, cond)
            mean, log_var = out["mean"], out["log_variance"]
    ...
    def training_losses(self, model, x_start, t, x_cond, model_kwargs=None, noise=None):
        ...
        mse_loss_weight = th.stack([snr, 5 * th.ones_like(t)], dim=1).min(dim=1)[0] / (snr + 1)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(th.cat([x_t,x_cond],dim=1), self._scale_timesteps(t), **model_kwargs)
